...
def _matrix4_to_threejs(matrix):
    """Flatten a 4x4 row-major numpy matrix to a three.js column-major list-16.

    three.js ``Matrix4.elements`` is column-major; the scene IR carries the
    conventional row-major ``M[row, col]`` layout, so a Fortran-order ravel
    (column 0, then column 1, ...) is exactly what three.js expects.
    """
    return np.asarray(matrix, dtype=np.float64).ravel(order='F').tolist()
...
def _instanced_shape_meshes(shape, face_color, edge_color, edge_width,
    ...
    The backend-neutral decomposition (which copies share a TShape, and the
    placement matrix of each) lives in ``_scene.shape_instance_groups``; this
    function only turns each group into pythreejs objects. Each periodic copy
    reuses the group's BufferGeometry / line geometries, with its placement
    applied as the object's model matrix; three.js uploads the shared geometry
    to the GPU only once and corrects normals via the per-object normal matrix.
    """
    face_meshes = []
    edge_lines = []

    for group in shape_instance_groups(shape, **extract_kwargs):
        template_face = faces_mesh(group.faces_data, color=face_color, opacity=face_opacity)
        template_edges = edges_mesh(group.edges_data, color=edge_color,
        ...
        for placement in group.matrices:
            matrix = _matrix4_to_threejs(placement)
