...
class TestBooleanCut:
    ...
    def _make_square_wire(self, z=-1.0):
        points = [
            ...
            mod3d.gp.Pnt(6.0, 4.0, z),
            mod3d.gp.Pnt(6.0, 6.0, z),
            mod3d.gp.Pnt(4.0, 6.0, z),
        ]
        edges = [
            mod3d.BRepBuilderAPI.MakeEdge(points[i], points[(i + 1) % len(points)]).edge()
            for i in range(len(points))
        ]
        return mod3d.BRepBuilderAPI.MakeWire(*edges).wire()
    ...
    def test_extrude_cut_face_profile(self):
        """Test extrude cut with a face profile."""
        box = mod3d.BRepBuilderAPI.MakeBox(10.0, 10.0, 10.0).shape()
        wire = self._make_square_wire()
        face = mod3d.BRepBuilderAPI.MakeFace(wire).face()

        result = mod3d.BooleanOp.extrude_cut(box, face, mod3d.gp.Vec(0.0, 0.0, 12.0))

        assert not result.is_null()

    def test_extrude_cut_wire_profile(self):
        """Test extrude cut with a wire profile."""
        box = mod3d.BRepBuilderAPI.MakeBox(10.0, 10.0, 10.0).shape()
        wire = self._make_square_wire()
